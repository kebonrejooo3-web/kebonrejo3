/*
  WebGIS Dusun Kebonrejo 3 - Fit & Layout Fix Version
  Pembaruan: Juli 2026
*/
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function highlightRTPolygon(rtNum) {
    ...
    rtLayerByNum.forEach((layer, num) => {
      if (num === rtNum) {
        layer.setStyle({ fillOpacity: 0.55, weight: 4.5 });
    ...
    .then((data) => {
      ...
      rtGeoJsonLayer = L.geoJSON(data, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          const categoriesInRT = [...new Set(itemsInRT.map((loc) => loc.kategori || loc.category))];
          ...
          let popupContent = `
            <div class="rt-popup">
              ...
              <div class="rt-popup-stat">
                <strong>${itemsInRT.length}</strong> Lokasi Terdaftar
              </div>
          `;

          if (categoriesInRT.length > 0) {
            popupContent += `<ul class="rt-popup-catlist">`;
            categoriesInRT.forEach((cat) => {
              const cfg = getCategoryConfig(cat);
              popupContent += `<li><span style="color:${cfg.color}; margin-right:4px;">${cfg.fallbackIcon}</span> ${escapeHtml(cat)}</li>`;
            });
            popupContent += `</ul>`;
          }
          ...
          layer.on({
            mouseover: (e) => {
              const l = e.target;
              l.setStyle({ fillOpacity: 0.50, weight: 4 });
              if (!L.Browser.ie && !L.Browser.opera && !L.Browser.edge) {
                l.bringToFront();
              }
            },
